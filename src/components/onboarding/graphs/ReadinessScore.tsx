...
export function ReadinessScore({ hours, weeksLeft }: Props) {
  ...
  const strokeColor =
    variant === 'success' ? '#22c55e' : variant === 'warning' ? '#f59e0b' : variant === 'danger' ? '#ef4444' : 'var(--accent-2)';

  return (
    <AnalyticsCard glow className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-[13px] font-semibold" style={{ color: 'var(--text-secondary)' }}>Readiness score</h4>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative w-24 h-24 shrink-0">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
            ...
            <motion.circle
              ...
              transition={prefersReducedMotion ? { duration: 0 } : { duration: 1.3, ease: 'easeOut' }}
            />
          </svg>
          <span className="absolute inset-0 flex items-center justify-center text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>
            {score}
          </span>
        </div>
        ...
        <div className="space-y-2 flex-1">
          <div className="flex justify-between text-xs">
            <span style={{ color: 'var(--text-dimmed)' }}>Hours/week</span>
            <span className="font-medium" style={{ color: 'var(--text-primary)' }}>{hours}h</span>
          </div>
          <div className="flex justify-between text-xs">
            <span style={{ color: 'var(--text-dimmed)' }}>Weeks remaining</span>
            <span className="font-medium" style={{ color: 'var(--text-primary)' }}>{weeksLeft}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span style={{ color: 'var(--text-dimmed)' }}>Total hours</span>
            <span className="font-medium" style={{ color: 'var(--text-primary)' }}>{hours * weeksLeft}h</span>
          </div>
          {risk && (
